...
Based on FR-009: Unity scene with real-time humanoid visualization synchronized with simulation
"""
...
class DigitalTwinSync(Node):
    ...
    def __init__(self):
        ...
        # Clock synchronization via /clock topic (from FR-009: clock sync via /clock)
        ...
        self.get_logger().info(
            f'Digital Twin Sync initialized:\n'
...
def main(args=None):
    """Main function to run the digital twin synchronization node."""
    rclpy.init(args=args)
    
    digital_twin_sync = DigitalTwinSync()
    
    try:
        rclpy.spin(digital_twin_sync)
    except KeyboardInterrupt:
        digital_twin_sync.get_logger().info('Shutting down Digital Twin Sync')
    finally:
        digital_twin_sync.destroy_node()
        rclpy.shutdown()
